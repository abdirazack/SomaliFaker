<?php

namespace SomaliFaker\Provider\so_SO;

class Text extends \Faker\Provider\Text
{
    protected static $baseText = array(
        'Soomaalidu been waa sheegtaa, been kuma maahmaahdo.',
        'Aan wada hadalno waa aan heshiino.',
        'Af aanad aqooni aabahaa wasna waa kaa yeedhsiiyaa.',
        'Af aanad lahayn ileen aamus lama dhaho.',
        'Af Daboolan, Dahab waaye.',
        'Belo markay adiga kaa maqan tahay, qayrkaa bay ku maqan tahay.',
        'Canbe laf baa ku jirta.',
        'Dab aan ku gubin kulaylkiisa ma ogid.',
        'Dab iyo xanuun meel ma wada fadhiistaan.',
        'Dad waxa yaqaan nin tuugsaday iyo nin u taliyey.',
        'Intaadan fallin ka fiirso.',
        'Idho jaceyl isma eegaan.',
        'Hadal waa margoo kale.',
        'Qof aan dhididini ma dhego.',
        'Gar iyo geeri loo siman.',
        'Rag haween ayaa kala hor mariya.',
        'Nin geed gali jirey geed loo ma galo.',
        'Naag la’aani waa naf la’aan.',
        'Talo xumo tog baas bay kaa riddaa.',
        'Nin is waalaba waabiyaa hela.',
        'Aaddane eed ma waayo.',
        'Haween la’aani waa hoy la’aan.',
        'Baahi badan, baryo badan, iyo bukaan badanba waa laysku nacaa.',
        'Gari Illaheed bay taqaan.',
        'Nin rag ah haweentii ka adag, doqonna isgaa ka adag.',
        'Barasho horteed ha I nicin.',
        'Waano abuur baa ka horeysay.',
        'Guri ann hooyo lahayni waa lama degaan.',
        'Rag waxaa u liita ma toshe, ma tashade iyo ma tashiishe.',
        'Been fakatay runi ma gaarto.',
        'Fari qorrax ma qariso.',
        'Af jooga loo adeego.',
        'Nin ann dhul marini dhaayo maleh.',
        'Meel waa laga muuqdaa ama waa laga maqnaadaa.',
        'Sirow ma hodmo.',
        'Samataliye sedkii waa janno.',
        'Xoolo rag farana ka tirsan feerana ka sisaaban.',
        'Runi rag kama nixiso.',
        'Qof kuu digey kuma dilin.',
        'Hadal badan haan ma buuxsho.',
        'Af qoyani hadal ma daayo.',
        'Sir naagood lama sal gaaro.',
        'Dagaal nin aan aqoon baa ku orda.',
        'Naago ama u samir, ama ka samir.',
        'Naag waa belaayo loo baahan yahay.',
        'Raggu waa nin naag guursaday iyo nin “naa! naa!” kororsaday.',
        'Cindhiga naageed iyo cir roobaad midna lama malayn karo.',
        'Kas dumar oo kadalloob rag kaaga dambeeyey kaad ku ogeyd ma aha.',
        'Dumar wax uga xuni ma jiraan “ama tag, ama joog”.',
        'Naago geel ka baxsaday nabastooday ka dayaan.',
        'Saqir naageed sirla’aan ma dhinto.',
        'Nin reer u weyn iyo naag naago ka ummulisay midna sida uu wax u og yahay uma sheego.',
        'Dumar nin ammaanaa leh.',
        'Dameeraha tan ugu foosha xun ayaa ugu haraanti kulul.',
        'Naago nin ay yaqaannaan col ma moodaan.',
        'Naagi nin ay taqaan nin ma mooddo.',
        'Hooyo xaarkay dhashay “Xaayow” tiraahdaa.',
        'Rag waa shaah, dumarna waa sheeko.',
        'Naago nin ay legdaan kama kacaan.',
        'Awr awr wado iyo nin naagi waddo midna kaama leexdo.',
        'Kal caano galeen kas ma galo.',
        'Nimanka iyo naagaha yaa badan?',
        'Naagaha ayaa badan, haddii lagu daro nimanka inta raacda ra’yiga naagaha.',
        'Naagla’aani waa nafla’aan.',
        'Nin aan haween lahayni hawl kama nasto.',
        'Jiilaalka iyo ninku way legdamayaan, kolba ka naagtu raacdaa adag.',
        'Rag naagaa is dhaafshay.',
        'Saddex ma gaadhid: Nin maalin ma gaadhid, ninna sannad ma gaadhid, ninna waligaaba ma gaadhid. Ninka kaa faras wanaagsan maalinta ma gaadhid. Ninka xoolihiisa meeshaada meel ka fiican daaqsadayna sannadka ma gaadhid. Ninka kaa naag wanaagsan weligaaba ma gaadhid.',
        'Geel laba jir soo wada mar.',
        'Luuq Luuq maroow laazim laamiga aas kusoo dhacaa.',
        'Wixii casirka lagaa arko aa cishahaa laguugu caaya.',
        'Waa rabaa roob aa lagu waaye.',
        'Ilko wada jir bey wax ku gooyaan.',
        'Aqoon la’aani waa iftiin la’aan.',
        'Beentaada hore runtaada dambe ayay dishaa.',
        'Barasho horteed ha i nicin.',
        'Far iyo meel bugtaa is og.',
        'Gabar nin dhali karaa ba dhiqi kara.',
        'Hadal nin si u yiri, ninna si u qaaday.',
        'Hadal yaar iyo hawl badaan.',
        'Labo qaawan isma qaado.',
        'Xarako xanuun ma leh.',
        'Ninkii lug isi gooyo usuga lee u hitinaayo.',
        'Ha dhiman, hadaad dhimato neh aaqiro ha dhaafin.',
        'Ninkii dad xume ku yiraah dad laawe aa la dhahaa.',
        'Dameer ceel ku dhacay kooy neh waa lagu dayyaa.',
        'Aqoon la’aani waa iftiin la’aan.',
        'Ilko wada jir bey wax ku gooyaan.',
        'Intaadan falin ka fiirso.',
        'Nabar doogi ma haro.',
        'Carruuri haddii aad kuftid way kugu qoslaan; haddii ay kufaanna way ooyaan.',
        'Doqonta usha agteeda ku dhufo.',
        'Nin aan waxa jooga garani waxa soo socda ma garto.',
        'Nin aanad saacad ku baran sannad kuma baratid.',
        'Nin walba midduu caashaqaa/ waa u Cambaro-luul.',
        'Dhurwaa kan ciya iyo kan aammusan, kan ciyaa roon.',
        'Dhurwaa kan ciya iyo kan aammusan, kee baa daran?',
        'Bannaankiisa-mare maradiisa geed ma qabsato.',
        'Caano aan fiiqsi loo dhamin iyo hadal aan fiiro loo odhan feedhahaaga ayay wax yeelaan.',
        'Meel lagaa jecel yahay haddaad ka tagto, meel lagaa neceb yahay ayaad tagtaa.',
        'Dhar magaalo intii laga xirto waa dhowdahay, dhal magaalse intii laga noqdo waa dheer tahay.',
    );
}
